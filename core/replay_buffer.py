...
class ReplayBuffer(object):
    ...
    def save_game(self, game, priorities=None):
        if priorities is None:
            ...
            assert len(game) == len(priorities), "priorites should be of same length as the game steps"
    ...
    def sample_batch(self, num_unroll_steps: int,  td_steps: int, beta: float = 1, model=None, config=None):
        ...
        for idx in indices:
            ...
            _action = game.history[game_pos:game_pos + num_unroll_steps]
            # random action selection to complete num unroll steps
            _actions += [np.random.randint(0, game.action_space_size)
                         for _ in range(num_unroll_steps - len(_action))]
    ...
    def remove_to_fit(self):
        if self.size() > self.soft_capacity:
            ...
            excess_games_steps = sum([len(game) for fame in self.buffer[:num_excess_games]])
