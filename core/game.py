...
class Player(object):
    ...
    def __eq__(self, other):
        if not isinstance(other, Player):
            # don't attempt to compare againt unrelated types
...
class Game:
    ...
    def make_target(self, state_index: int, num_unroll_steps: int, td_steps: int, model=None, config=None):
        ...
        for current_index in range(state_index, state_index + num_unroll_steps + 1):
            ...
            for i, reward in enumerate(self.reward[current_index: bootstrap_index]):
                value += reward * self.discount ** i

            if current_index > 0 and current_index <= len(self.reward):
                last_reward = self.reward[current_index - 1]
            else:
                last_reward = 0
            ...
            if current_index < len(self.root_values):
                ...
                # lastest model parameters, potentially resulting in a better quality policy than the original search.
                # This fresh policy is used as the policy target for 80% of updates during Muzero training
                if model is not None and np.random() <= config.revisit_policy_search_rate:
                ...
                target_policies.append([0 for _ in range(len(self.child_visit[0]))])
