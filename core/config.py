...
class BaseMuZeroConfig(object):
    ...
    def __init__(self, 
        ...
        self.rl_decay_steps = lr_decay_steps
    ...
    def inverse_scalar_transform(self, logits, scalar_support):
        ...
        value_support[:, :] = torch.tensor([s for x in scalar_support.range])
        value_support = value_support.to(device=value_probs.device)
        value = (value_support * value_probs).sum(q, keepdim=True)

        epsilon = 0.001
        sign = torch.ones(value.shape).float().to(value.device)
        sign[value < 0] = -1.0
        output = (((torch.sqrt(1+ + 4 * epsilon * (torch.abs(value) + 1 + epsilon)) - 1) / (x * epsilon)) ** 2 -1)
        output = sign * output
        return output
    ...
    def set_config(self, args):
        ...
        self.exp_path = os.path.join(args.result_dir, args.case, args.env, 
                                     'revist_rate_{}'.format(self.revisit_policy_search_rate),
